...
class EightPuzzle:
    ...
    def _get_possibleMoves(self):
        ...
        if row > 0: #can swap left
            free.append((row - 1, col))
        if col > 0: #can swap down
    ...
    def randomize(self, step_count):
        #randomizer that allows user to define the number of times it randomizes
    ...
    def getValofCoord(self, row, col):
    ...
    def swap(self, pos_a, pos_b):
        ...
        temp = self.getValofCoord(*pos_a)
        self.poke(pos_a[0], pos_a[1], self.getValofCoord(*pos_b))
        self.poke(pos_b[0], pos_b[1], temp)


def getHeur(puzzle, item_total_calc, total_calc):
    """
    A function that returns the cost of the heuristic
    """
    t = 0
    for row in range(3):
        for col in range(3):
            val = puzzle.getValofCoord(row, col) - 1
            target_col = val % 3
            target_row = val / 3
            ...
            #depending on what item_total_cal is defined as in lambda, item_total_calc(row, target_row, col, target_col) returns a different value
...
def index(item, seq):
    ...
    if item in seq:
        ...
        return -1
def heur_distance(puzzle):
    return getHeur(puzzle,
...
def main():
    ...
    path, count = p.solve(heur_distance)
    path.reverse()
    count = 0;
    for i in path: 
        print 'step:',count
